...
#include <symbol_attr.h>
    ...
    *********************************/
    ...
    EXPORT_SYMBOL void debug_initialize()
    {
    ...
    EXPORT_SYMBOL void debug_printf(const char* message, ...)
    {
        va_list args;
        va_start(args, message);
        debug_vprintf(message, args);
        va_end(args);
    }
    ...
    /*==============================
        ...
        Supports up to 256 characters.
        @param A string to print
        @param list for variadic arguments
    ==============================*/
    
    EXPORT_SYMBOL void debug_vprintf(const char* message, va_list args)
    {
        int len = 0;
        usbMesg msg;
        ...
        len = vsnprintf(debug_buffer, BUFFER_SIZE-1, message, args);
    ...
    EXPORT_SYMBOL void debug_dumpbinary(void* file, int size)
    {
        usbMesg msg;
    ...
    EXPORT_SYMBOL void debug_screenshot()
    {
        usbMesg msg;
        int data[4];
    ...
    EXPORT_SYMBOL void _debug_assert(const char* expression, const char* file, int line)
    {
        volatile char crash;
    ...
    EXPORT_SYMBOL void debug_addcommand(char* command, char* description, char* (*execute)())
    {
    ...
    EXPORT_SYMBOL void debug_printcommands()
    {
        int i;
    ...
    EXPORT_SYMBOL void debug_pollcommands()
    {
        usbMesg msg;
    ...
    EXPORT_SYMBOL int debug_sizecommand()
    {
    ...
    EXPORT_SYMBOL void debug_parsecommand(void* buffer)
    {
                ...
                while (1)
                {
                    ...
                    if (curr != NULL) 
                    {
                        ...
                        if (assert_file == NULL) {
							...
							debug_printf("cause\t\t");
							debug_printf("Assertion '%s' failed in file '%s', line %d.\n", assert_expr, assert_file, assert_line);
						}
                        debug_printreg(context->sr, "sr", srDesc);
                        debug_printf("badvaddr\t0x%08x\n\n", context->badvaddr);
                        
						USOHandle curr_handle = USOGetFirstHandle();
						debug_printf("---------------------------Loaded USOs---------------------------\n");
						debug_printf("Handle   Min_Addr-Max_Addr Name(Reference Count)\n");
						if(curr_handle) {
							while(curr_handle) {
								USOHandleInfo info = USOGetHandleInfo(curr_handle);
								debug_printf("%08x %08x-%08x %s(%d)\n", info.handle, info.min_addr, info.max_addr,
									info.name, info.ref_count);
								curr_handle = USOGetNextHandle(curr_handle);
							}
						} else {
							debug_printf("-------------------------No Loaded USOs--------------------------\n");
						}
