...
// Function pointers
static void (*funcPointer_write)(int datatype, const void* data, int size);
static u32  (*funcPointer_poll)();
static void (*funcPointer_read)();
...
EXPORT_SYMBOL char usb_initialize()
{
...
EXPORT_SYMBOL char usb_getcart()
{
    return usb_cart;
}
...
EXPORT_SYMBOL void usb_write(int datatype, const void* data, int size)
{
...
EXPORT_SYMBOL void usb_read(void* buffer, int nbytes)
{
    int read = 0;
    int left = nbytes;
...
EXPORT_SYMBOL void usb_skip(int nbytes)
{
...
EXPORT_SYMBOL void usb_rewind(int nbytes)
{
